...
public class UserController {
	...
	public ResponseEntity<User> addFavourite(@PathVariable long userId, @PathVariable long courseId) {
		...
		if (hasCourse) return new ResponseEntity<User>(user, HttpStatus.OK);
		
		// Check if course exists
		boolean courseExists = courseRepository.existsById(courseId);
		if (!courseExists) return new ResponseEntity<User>(user, HttpStatus.BAD_REQUEST);
		
		// Add course to user favourites
		FavouriteCourse favouriteCourse = favouriteCourseRepository.save(new FavouriteCourse(favouriteCourseRepository.count()+1, userId, courseId));
	...
	public ResponseEntity<User> deleteFavourite(@PathVariable long userId, @PathVariable long courseId) {
		...
		boolean hasCourse = favourites.stream().anyMatch(course -> course.getCourse().getCourseId() == courseId);
		if (!hasCourse) return new ResponseEntity<User>(user, HttpStatus.BAD_REQUEST);
		
		// Remove course from user favourites
		Optional<FavouriteCourse> favouriteCourse = favourites.stream().filter(course -> course.getCourse().getCourseId() == courseId).findFirst();
	...
	public ResponseEntity<User> addSubscription(@PathVariable long userId, @PathVariable long courseId) {
		...
		SubscribedCourse subscribedCourse = subscribedCourseRepository.save(new SubscribedCourse(subscribedCourseRepository.count()+1, userId, courseId, 0, 'P'));
	...
	public ResponseEntity<User> deleteSubscription(@PathVariable long userId, @PathVariable long courseId) {
		...
		Optional<SubscribedCourse> subscribedCourse = subscriptions.stream().filter(course -> course.getCourse().getCourseId() == courseId).findFirst();
